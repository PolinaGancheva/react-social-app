import React from 'react';
import { PostModel, CommentModel } from 'types';
import { useMutation } from 'react-query';
import { useAuth } from 'hooks/domain';
import * as devcamp from 'api/devcamp';
import { updateInCache, removeFromCache } from 'utils/cache';
import { Dropdown, DropdownButton } from 'react-bootstrap';

type Props = {
  post: PostModel;
  comment: CommentModel;
};

const CommentMenu = ({ post, comment }: Props) => {
  const { user } = useAuth();
  const [removeComment, { status: removeStatus }] = useMutation(
    devcamp.deleteComment,
    {
      onSuccess: () => {
        removeFromCache(['postComments', post.id], comment.id);
        updateInCache<PostModel>(['posts'], post.id, (old) => ({
          ...old,
          commentsCount: old.commentsCount - 1,
        }));
        updateInCache<PostModel>(
          ['userPosts', post.user.id],
          post.id,
          (old) => ({
            ...old,
            commentsCount: old.commentsCount - 1,
          })
        );
      },
    }
  );

  if (comment.user.id !== user?.id && post.user.id !== user?.id) {
    return null;
  }

  const isLoading = removeStatus === 'loading';

  return (
    <Dropdown
      color="link"
      style={{ display: 'inline-flex', marginLeft: '0.5rem' }}
    >
      <DropdownButton
        variant="link"
        id="dropdown-item-button"
        title={
          <img
            src="data:image/png;base64,"
            alt="dots"
            style={{
              width: '16px',
              height: '16px',
            }}
          />
        }
      >
        <Dropdown.Item
          as="button"
          onClick={isLoading ? undefined : () => removeComment(comment.id)}
        >
          Remove
        </Dropdown.Item>
      </DropdownButton>
    </Dropdown>
  );
};
export default CommentMenu;
